    ...
    <div style={styles.container_info_prd}>
      <div style={styles.header_slider}>
        <div style={{ display: 'flex' }}>
          <Button className="custom-cursor" variant="outlined" sx={styles.btn_media}>
            <CameraAltIcon />
            <Typography sx={{ marginLeft: '6px' }}>
              {idxImage} / {data.img.length}
            </Typography>
          </Button>
          <Button variant="outlined" disabled={true} sx={Object.assign({ marginLeft: '12px' }, styles.btn_media)}>
        ...
        <div style={styles.body_slider}>
          <Carousel
            ...
            renderThumbs={arr_img =>
              arr_img.map((elm, index) => <img key={index} src={elm.props.children.props.src} alt="img thumb" />)
            }
          >
