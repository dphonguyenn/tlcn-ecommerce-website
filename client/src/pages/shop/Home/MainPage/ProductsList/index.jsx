  ...
  const ToastContent = () => (
    ...
    >
      ...
      <Button sx={{ marginLeft: "24px" }} onClick={() => navigate("/cart")}>
        <Typography sx={{ textTransform: "initial", color: "#66f1ff" }}>
          Xem giỏ hàng
        </Typography>
      </Button>
    </div>
          ...
          return (
            <Grid key={device._id} item xs={6} md={4}>
              ...
              <Card
                sx={styles.card}
                onClick={() =>
                  redirectDetailedProductPage(device.type, device._id)
                }
                onMouseOver={() => handleHoverCard(device._id)}
                onMouseLeave={() => handleHoverCard("")}
              >
                {showGroupBtnCard === device._id ? (
                  <GroupBtnCard data={device} />
                ) : (
                  <></>
                )}
                <div style={{ padding: "16px 40px" }}>
                  <CardMedia
                    sx={styles.card_media}
                    component="img"
                    alt="card media"
                    image={device.img[0]}
                  />
                    ...
                    <div>
                      ...
                      {device.sale > 0 && (
                        <div style={styles.card_footer}>
                          ...
                          <Typography sx={styles.subtitle4_font}>
                            Giảm giá
                          </Typography>
                        </div>
                      )}
