...
const {data, refetch} = useGetLapTop(state.selectedType);
  ...
  useEffect(()=>{
    const rows = data?.rows?.map(r =>{
      return {id: r._id, name: r.name, originalPrice: r.original_price, sale:r.sale, sale_price:r.sale_price, quantity:r.quantity }
    })
    setState({
      ...state,
      rows:rows
  ...
  return(
    <div className='productMainAdmin'>
        ...
        <select value={state.selectedType} onChange={(e)=>{setState({...state,selectedType:e.target.value})}} className='select'> 
        {listType?.map((s, index) => {
            return (
            <option className="selected" key={index} value={s.id}>{s.name}</option>
            )
        })}
      </select>
        </div>
